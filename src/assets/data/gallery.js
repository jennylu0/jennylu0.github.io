const imgPath = '/gallery/';
const data = [
    {
        img: `${imgPath}Phm-sm.png`,
        filters: ["illustration"],
    },
    {
        img: `${imgPath}Re9-grabby.png`,
        filters: ["illustration"],
    },
    {
        img: `${imgPath}Dungeon-crawler-Carl.png`,
        filters: ["illustration"],
    },
    {
        img: `${imgPath}Isekai.png`,
        filters: ["illustration"],
    },
    {
        img: `${imgPath}Ryan-goslings-sm.png`,
        filters: ["merch"],
        props: ["sticker"],
        modifier: 'sticker'
    },
    {
        img: `${imgPath}Patterns-formatted-dcc.png`,
        filters: ["pattern"],
        props: ["pattern"]
    },
    {
        img: `${imgPath}Patterns-formatted-kuroshitsuji.png`,
        filters: ["pattern"],
        props: ["pattern"]
    },
    {
        img: `${imgPath}Murderbot.png`,
        filters: ["illustration"],
    },
    {
        img: `${imgPath}Patterns-formatted-miku.png`,
        filters: ["pattern"],
        props: ["pattern"]
    },
    {
        img: `${imgPath}Patterns-formatted-phm.png`,
        filters: ["pattern"],
        props: ["pattern"]
    },
    {
        img: `${imgPath}Leons-sticker2-sm.png`,
        filters: ["merch"],
        props: ["sticker"],
        modifier: 'sticker'
    },
    {
        img: `${imgPath}Phantom-thief-kid.png`,
        filters: ["illustration"],
    },
    {
        img: `${imgPath}Patterns-formatted-fruit.png`,
        filters: ["pattern", "original"],
        props: ["pattern"]
    },
    {
        img: `${imgPath}Fight-like-a-magical-girl.png`,
        filters: ["illustration"],
    },
    {
        img: `${imgPath}Miku-screenprint.png`,
        filters: ["illustration"],
    },
    {
        img: `${imgPath}Dungeon-meshi-wood-sticker.png`,
        filters: ["merch"],
        props: ["sticker"],
        modifier: 'sticker'
    },
    // {
    //     img: `${imgPath}Anime-lucky-cats-sm.png`,
    //     filters: ["merch"],
    // },
    {
        img: `${imgPath}Patterns-formatted-xiao.png`,
        filters: ["pattern"],
        props: ["pattern"]
    },
     {
        img: `${imgPath}Red-head-girls.png`,
        filters: ["illustration"],
    },
    // {
    //     img: `${imgPath}poke-rainbow.jpeg`,
    //     filters: ["illustration"],
    // },
        {
        img: `${imgPath}mew-menu.png`,
        filters: ["illustration"],
    },
    {
        img: `${imgPath}Patterns-formatted-tmm.png`,
        filters: ["pattern"],
        props: ["pattern"]
    },
    {
        img: `${imgPath}Spotify-charms.png`,
        filters: ["merch"],
    },
    // {
    //     img: `${imgPath}Fruit-stickers.png`,
    //     filters: ["merch"],
    // },
    // {
    //     img: `${imgPath}mailedit.png`,
    //     filters: ["illustration", "original"],
    // },
    // {
    //     img: `${imgPath}acnh-cards.png`,
    //     filters: ["merch"],
    // },
    // {
    //     img: `${imgPath}angels-share.png`,
    //     filters: ["illustration"],
    // },
    // {
    //     img: `${imgPath}Ccs-Sakura-tomoyo.png`,
    //     filters: ["illustration"],
    // },
    {
        img: `${imgPath}Denji-sticker-sm.png`,
        filters: ["merch"],
        props: ["sticker"],
        modifier: 'sticker'
    },
    // {
    //     img: `${imgPath}Diona-Klee-sneakers.png`,
    //     filters: ["illustration"],
    // },
    {
        img: `${imgPath}Dungeon-meshi-kitchen-conversion.png`,
        filters: ["illustration"],
    },
    // {
    //     img: `${imgPath}kaeya-knight.png`,
    //     filters: ["illustration"],
    // },
    {
        img: `${imgPath}Slime-mochi-donut-sm.png`,
        filters: ["merch"],
        props: ["sticker"],
        modifier: 'sticker'
    },
    {
        img: `${imgPath}Genshin-lolita.png`,
        filters: ["illustration"],
    },
    {
        img: `${imgPath}genshin-periodic-table.png`,
        filters: ["illustration"],
    },
  
    //  {
    //     img: `${imgPath}Sticker-sheet-old-consoles.png`,
    //     filters: ["merch"],
    // },
    {
        img: `${imgPath}Patterns-formatted-ccs.png`,
        filters: ["pattern"],
        props: ["pattern"]
    },
    // {
    //     img: `${imgPath}hualian-ch-224.png`,
    //     filters: ["illustration"],
    // },
    // {
    //     img: `${imgPath}wwx-5-star.png`,
    //     filters: ["illustration"],
    // },
    {
        img: `${imgPath}Patterns-formatted-mao.png`,
        filters: ["pattern", "original"],
        props: ["pattern"],
    },
    // {
    //     img: `${imgPath}hxh.png`,
    //     filters: ["illustration"],
    // },
    {
        img: `${imgPath}Kumoricon-2024.png`,
        filters: ["illustration", "original"],
    },
    // {
    //     img: `${imgPath}Persona5-sticker-sm.png`,
    //     filters: ["merch"],
    //     props: ["sticker"],
    //     modifier: 'sticker'
    // },
    {
        img: `${imgPath}Patterns-formatted-p5.png`,
        filters: ["pattern"],
        props: ["pattern"]
    },
    {
        img: `${imgPath}Kuroshitsuji-fashion.png`,
        filters: ["illustration"],
    },
    // {
    //     img: `${imgPath}pattern-scrunchies.png`,
    //     filters: ["merch"],
    // },
    {
        img: `${imgPath}Patterns-formatted-conan.png`,
        filters: ["pattern"],
        props: ["pattern"]
    },
    {
        img: `${imgPath}Patterns-formatted-kid.png`,
        filters: ["pattern"],
        props: ["pattern"]
    },
    // {
    //     img: `${imgPath}sticker-sheet-kimetsu.png`,
    //     filters: ["merch"],
    // },
    // {
    //     img: `${imgPath}strawberry-kaeluc.png`,
    //     filters: ["illustration"],
    // },
    // {
    //     img: `${imgPath}Sticker-sheet-stamps.png`,
    //     filters: ["merch"],
    // }
];

export default data;
